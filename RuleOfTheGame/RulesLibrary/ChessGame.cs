...
namespace ChessDotNet
{
    public class ChessGame
    {
        ...
        Dictionary<string, Int16> MovesList = new Dictionary<string, Int16>();
        public void InsertMove(string s)
        {
            s = s.Remove(s.Length - 4);
            if (MovesList.ContainsKey(s))
            {
                MovesList[s] += 1;
            }
            else { MovesList.Add(s, 1); }
        }
        public bool IsItDrawByTRR(string s)
        {
            s = s.Remove(s.Length - 4);
            if (MovesList[s] >= 3)
            {
        ...
        public bool Terminated
        {
            get
            {
                return (DrawClaimed || IsCheckmated(Player.Black) || IsCheckmated(Player.White));
            }
        }
